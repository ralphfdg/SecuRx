const initDispense = () => {
    window.Alpine.data(
        "dispenseController",
        (prescriptionId, availableItems, submitUrl, requiresOverride) => ({
            isSubmitting: false,
            isSuccess: false,
            errorMessage: "",
            ...
            get isFormValid() {
                ...
                if (this.is_proxy && !this.representative_id) return false;
                if (this.selectedItems.length === 0) return false;

                const hasEmptyFields = this.selectedItems.some(
                    (item) =>
                        !item.actual_drug_dispensed ||
                ...
                if (requiresOverride && !this.global_override_reason.trim())
                    return false;
