...
@section('content')
    @php
        ...
        // FIX: Triage vitals belong to the Appointment, not the Encounter!
        $rawVitals = $encounter->appointment->triage_vitals ?? null;
        $vitals = is_string($rawVitals) ? json_decode($rawVitals, true) : $rawVitals ?? [];

        // Grab the primary prescription for this encounter
        $rx = $encounter->prescriptions->first();

        // Grab the doctor profile for the header
        $doctorProfile = auth()->user()->doctorProfile;
    @endphp

    <div x-data="{ showRevokeModal: false }" class="max-w-7xl mx-auto space-y-4 pb-12">

        <div
            class="bg-white/90 backdrop-blur-md border border-gray-200 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <div class="flex items-center gap-3 mb-0.5">
                        <h2 class="text-lg font-black text-securx-navy leading-none">Consultation Record</h2>
                    ...
                    <p class="text-xs text-gray-500 font-medium">
                        Date: {{ \Carbon\Carbon::parse($encounter->encounter_date)->format('M d, Y • h:i A') }} •
                        Ref: {{ strtoupper(substr($encounter->id, 0, 8)) }}
                    </p>
                </div>
            </div>
        ...
        <div class="flex flex-col lg:flex-row gap-6 items-start">
            ...
            <div class="w-full lg:w-7/12 space-y-4">
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
                    <h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Patient Profile</h3>
                    <div class="flex items-center gap-4 mb-3 border-b border-gray-100 pb-3">
                        <div
                            class="w-10 h-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-black text-lg shrink-0">
                            {{ $initials }}
                        </div>
                        <div>
                            <p class="font-black text-securx-navy text-base leading-none">{{ $patient->last_name }},
                                {{ $patient->first_name }}</p>
                            <p class="text-xs text-gray-500 font-medium mt-1">ID: {{ substr($patient->id, 0, 8) }} •
                                {{ $age }} yrs • {{ $patient->gender ?? 'Unknown' }}</p>
                        </div>
                    </div>

                    <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 bg-slate-50 p-3 rounded-xl border border-gray-100">
                        <div>
                            <p class="text-[9px] font-bold text-gray-400 uppercase mb-0.5">BP</p>
                            <p class="text-sm font-black text-red-500">{{ $vitals['blood_pressure'] ?? '--' }}</p>
                        </div>
                        <div>
                            <p class="text-[9px] font-bold text-gray-400 uppercase mb-0.5">Temp</p>
                            <p class="text-sm font-black text-securx-navy">
                                {{ $vitals['temperature'] ?? '--' }}{{ isset($vitals['temperature']) ? '°C' : '' }}</p>
                        </div>
                        <div>
                            <p class="text-[9px] font-bold text-gray-400 uppercase mb-0.5">HR</p>
                            <p class="text-sm font-black text-red-500">{{ $vitals['heart_rate'] ?? '--' }}</p>
                        </div>
                        <div>
                            <p class="text-[9px] font-bold text-gray-400 uppercase mb-0.5">Wt</p>
                            <p class="text-sm font-black text-securx-navy">
                                {{ $vitals['weight'] ?? ($profile->weight ?? '--') }}{{ isset($vitals['weight']) || isset($profile->weight) ? 'kg' : '' }}
                            </p>
                        </div>
                        <div>
                            <p class="text-[9px] font-bold text-gray-400 uppercase mb-0.5">Ht</p>
                            <p class="text-sm font-black text-securx-navy">
                                {{ $vitals['height'] ?? ($profile->height ?? '--') }}{{ isset($vitals['height']) || isset($profile->height) ? 'cm' : '' }}
                            </p>
                        </div>
                        <div class="col-span-2 sm:col-span-4 mt-1 pt-2 border-t border-gray-200">
                            <p class="text-[9px] font-bold text-gray-400 uppercase mb-0.5">Triage Note</p>
                            <p class="text-xs font-medium text-gray-700 italic">{{ $vitals['notes'] ?? '--' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div class="p-4 space-y-3">
                        <div>
                            <p class="text-[10px] font-bold text-blue-600 uppercase tracking-wider mb-1">Subjective</p>
                            <div
                                class="bg-slate-50 border border-gray-100 p-2.5 rounded-lg text-xs text-gray-700 font-medium leading-relaxed">
                                {{ $encounter->subjective_note ?? 'No subjective notes recorded.' }}
                            </div>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-emerald-600 uppercase tracking-wider mb-1">Objective</p>
                            <div
                                class="bg-slate-50 border border-gray-100 p-2.5 rounded-lg text-xs text-gray-700 font-medium leading-relaxed">
                        ...
                        <div>
                            <p class="text-[10px] font-bold text-purple-600 uppercase tracking-wider mb-1">Assessment</p>
                            <div
                                class="bg-slate-50 border border-gray-100 p-2.5 rounded-lg text-xs text-gray-700 font-medium leading-relaxed">
                        ...
                        <div>
                            <p class="text-[10px] font-bold text-amber-600 uppercase tracking-wider mb-1">Plan</p>
                            <div
                                class="bg-slate-50 border border-gray-100 p-2.5 rounded-lg text-xs text-gray-700 font-medium leading-relaxed">
            ...
            <div class="w-full lg:w-5/12 flex flex-col items-center relative">
                ...
                <div
                    class="bg-white shadow-xl border border-gray-200 relative flex flex-col w-full max-w-[148mm] aspect-[1/1.414] rounded-sm overflow-hidden {{ $rx && $rx->status === 'revoked' ? 'opacity-70 grayscale' : '' }}">
                    ...
                    <div class="p-5 flex-1 flex flex-col relative z-10 min-h-0">
                        ...
                        <div class="flex flex-row items-center gap-3 border-b-2 border-gray-800 pb-3 mb-3 shrink-0">
                            ...
                            <div class="flex-1 flex flex-col min-w-0">
                                ...
                                <div class="flex flex-row justify-between items-start gap-1 w-full">
                                    <p
                                        class="text-[9px] sm:text-[10px] font-serif text-gray-600 leading-snug flex-1 text-left">
                                        {{ $doctorProfile->clinic->clinic_address ?? '123 Health Avenue, Medical District, City' }}
                                    </p>
                                    <p
                                        class="text-[9px] sm:text-[10px] font-serif text-gray-800 font-bold shrink-0 text-right whitespace-nowrap">
                        ...
                        <div
                            class="grid grid-cols-4 gap-2 text-[9px] font-serif text-gray-800 mb-3 border-b border-gray-300 pb-2 shrink-0">
                            <div class="col-span-2">
                                <p class="text-[7px] font-bold text-gray-500 uppercase">Name:</p>
                                <p
                                    class="font-bold border-b border-gray-300 border-dotted truncate {{ !$rx || !$rx->print_patient_name ? 'text-gray-400 italic' : 'text-gray-900' }}">
                                    {{ $rx && $rx->print_patient_name ? $patient->name ?? 'Unknown' : '____________________' }}
                                </p>
                            </div>
                            <div class="col-span-1">
                                <p class="text-[7px] font-bold text-gray-500 uppercase">Age/Sex:</p>
                                <p
                                    class="font-bold border-b border-gray-300 border-dotted {{ !$rx || !$rx->print_patient_name ? 'text-gray-400 italic' : 'text-gray-900' }}">
                                    @if ($rx && $rx->print_patient_name)
                                        {{ $age }}/{{ strtoupper(substr($patient->gender ?? 'U', 0, 1)) }}
                                    @else
                                        __________
                                    @endif
                                </p>
                        ...
                        <div
                            class="flex-1 space-y-3 pl-1 font-serif overflow-y-auto custom-scrollbar pr-2 flex flex-col relative min-h-[150px]">
                                    ...
                                    <div class="relative group">
                                        <p class="text-xs font-bold text-gray-900 flex items-center flex-wrap gap-1.5">
                                            ...
                                            @if ($item->medication->brand_name)
                                                <span
                                                    class="text-gray-500 italic text-[10px] font-normal">({{ $item->medication->brand_name }})</span>
                                            @endif
                                            @if ($item->medication->dosage_strength)
                                                <span
                                                    class="bg-red-100 text-red-700 border border-red-200 font-black px-1 py-0.5 rounded text-[9px] uppercase tracking-wider shadow-sm">{{ $item->medication->dosage_strength }}</span>
                                            @endif
                                            @if ($item->medication->form)
                                                <span
                                                    class="text-[10px] text-gray-500 font-medium">{{ $item->medication->form }}</span>
                                            @endif
                                        </p>
                                        <div class="flex justify-between items-end mt-0.5">
                                            <p class="text-[11px] text-gray-700 italic">Sig:
                                                {{ $item->sig ?: 'Take as directed' }}</p>
                        ...
                        @if ($rx && ($rx->general_instructions || $encounter->next_appointment_date))
                                ...
                                @if ($encounter->next_appointment_date)
                                    <div>
                                        ...
                                        <p class="text-[10px] font-serif text-gray-900 mt-0.5 font-bold">
                                            {{ \Carbon\Carbon::parse($encounter->next_appointment_date)->format('F j, Y') }}
                                        </p>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="flex justify-between items-end">
                                ...
                                @if ($rx)
                                    <div class="flex flex-col items-center shrink-0">
                                        <div
                                            class="w-24 h-24 bg-white border border-gray-200 p-1 flex items-center justify-center rounded">
                                            {!! \SimpleSoftwareIO\QrCode\Facades\QrCode::size(86)->margin(0)->generate($rx->id) !!}
                                        </div>
                                        <span
                                            class="text-[7px] text-gray-400 font-mono mt-1 tracking-widest uppercase">UUID:
                                            {{ substr($rx->id, 0, 13) }}...</span>
                                    </div>
                                @else
                                    <div
                                        class="w-24 h-24 bg-gray-50 border border-gray-200 flex flex-col items-center justify-center shrink-0 rounded">
                                        ...
                                        <span class="text-[8px] text-gray-400 mt-1 font-sans">NO RX</span>
                                    </div>
                                @endif
                                ...
                                <div class="text-left w-[140px] ml-auto flex flex-col font-serif">
                                    <div class="relative z-0 space-y-1 text-[9px]">
                                        <div class="relative">
                                            ...
                                            <span
                                                class="absolute left-0.5 bottom-0 font-sans font-bold text-blue-800 uppercase opacity-90 truncate max-w-[120px] inline-block">{{ auth()->user()->name }}</span>
                                        </div>
                                        <div class="relative">
                    ...
                    <div x-show="showRevokeModal" x-transition:enter="ease-out duration-300"
                        ...
                        <form method="POST" action="{{ route('doctor.history.revoke', $rx->id) }}">
                            ...
                            <div class="px-4 py-5 sm:p-6 space-y-4">
                                <div>
                                    <label
                                        class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-2">Reason
                                ...
                                <div>
                                    <label
                                        class="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-2">Security
                                        Authorization</label>
