...
<div class="max-w-6xl mx-auto space-y-6 relative">
    ...
    <div class="glass-panel overflow-hidden bg-white/80 flex flex-col border-t-4 border-t-securx-cyan shadow-sm rounded-2xl">
        <div class="p-4 border-b border-gray-200/60 bg-gray-50/90 backdrop-blur-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <form method="GET" action="{{ route('admin.specializations') }}" class="relative w-full md:w-96">
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Search specialties..." 
...
<script>
    // Modals
    function openAddSpecModal() { document.getElementById('add-spec-modal').classList.remove('hidden'); }
    function closeAddSpecModal() { document.getElementById('add-spec-modal').classList.add('hidden'); }

    function openEditSpecModal(id, name, desc) {
    ...
    function closeEditSpecModal() { document.getElementById('edit-spec-modal').classList.add('hidden'); }
    ...
    function closeDeleteSpecModal() { document.getElementById('delete-spec-modal').classList.add('hidden'); }
</script>
@endpush
@endsection