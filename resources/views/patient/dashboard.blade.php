        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="glass-panel p-6 bg-white/60 flex items-center gap-4 hover:-translate-y-1 transition-transform">
                ...
                <div>
                    ...
                    <p class="text-2xl font-extrabold text-securx-navy">{{ $activePrescriptionsCount }}</p>
                </div>
            </div>
            ...
            <div class="glass-panel p-6 bg-white/60 flex items-center gap-4 hover:-translate-y-1 transition-transform">
                ...
                <div>
                    ...
                    <p class="text-2xl font-extrabold text-securx-navy">{{ $user->patientEncounters()->count() }}</p>
                </div>
            </div>
            ...
            <div class="glass-panel p-6 bg-white/60 flex items-center gap-4 hover:-translate-y-1 transition-transform">
                ...
                <div>
                    ...
                    <p
                        class="text-lg font-extrabold {{ $user->patientProfile?->data_consent ? 'text-green-600' : 'text-securx-gold' }}">
                        {{ $user->patientProfile?->data_consent ? 'Active' : 'Pending' }}
                    </p>
                </div>
            </div>
        </div>
                                ...
                                <td class="p-4">
                                    <p class="font-bold text-securx-navy">
                                        ...
                                        @if ($prescription->items->count() > 1)
                                            <span class="text-xs text-securx-cyan">+{{ $prescription->items->count() - 1 }}
                                                more</span>
                                        @endif
                                    </p>
                                    <p class="text-xs text-gray-500">
                                        {{ $prescription->items->first()->sig ?? 'View prescription for instructions' }}
                                    </p>
                                </td>
                                <td class="p-4 font-medium">Dr. {{ $prescription->doctor->last_name }}</td>
                                <td class="p-4 text-gray-500">{{ $prescription->created_at->format('M d, Y') }}</td>
                                <td class="p-4">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="p-8 text-center text-gray-500">
                                    <p class="mb-2">No active prescriptions found.</p>
                                    <p class="text-xs">Any new prescriptions issued by your doctor will appear here
                                        automatically.</p>
                                </td>
                            </tr>
                        @endforelse
